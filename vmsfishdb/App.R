...
##function to check the integrity of the new uploaded rds file

check_rds_format <- function(rdsfile) {
  # Essential elements
  essential_dfs <- c("nursery_info", "adult_info", "Archive", "Database_logs", "user_base")
  essential_info_colname <- c("RoomName", "db_list_name", "Permission_to_edit")
  essential_userbase_colname <- c("user", "password", "permissions", "name")
  essential_nursery_colname <- c("No..of.Tanks", "Stock.number.", "Date.of.Birth",
                                 "Fish.name", "Genotype.", "Responsible",
                                 "Number.of.fish", "Notes")
  essential_adult_colnames <- c("Tank.Name", "Stock.number.", "Date.of.Birth", "Fish.name",
                                "Genotype.", "Responsible", "Number.of.fish",
                                "Experiment.label", "Food.label", "Notes")
  
  # Check if essential data frames exist
  if (!all(essential_dfs %in% names(rdsfile))) {
    return(FALSE)
  }
  # Check columns in nursery_info
  if (!all(essential_info_colname %in% colnames(rdsfile$nursery_info))) {
    return(FALSE)
  }
  # Check columns in adult_info
  if (!all(essential_info_colname %in% colnames(rdsfile$adult_info))) {
    return(FALSE)
  }
  ...
  if (length(colnames(rdsfile$Archive)) != 12 || length(colnames(rdsfile$Database_logs)) != 8) {
    return(FALSE)
  }
  # Check columns in user_base
  if (!all(essential_userbase_colname %in% colnames(rdsfile$user_base))) {
    return(FALSE)
  }
  # Check if the first user is "Administrator"
  if (rdsfile$user_base$user[1] != "Administrator") {
    return(FALSE)
  }
  # Check columns in specific nursery database
  if (!all(essential_nursery_colname %in% colnames(rdsfile[[rdsfile$nursery_info$db_list_name[1]]]))) {
    return(FALSE)
  }
  # Check columns in specific adult database
  if (!all(essential_adult_colnames %in% colnames(rdsfile[[rdsfile$adult_info$db_list_name[1]]]))) {
    return(FALSE)
  }
  return(TRUE)
}
...
ui<- dashboardPage(
  #dashboardHeader(title = "VMS FISH DATABASE", titleWidth = 300 ,tags$li(class="dropdown",style="padding:8px;", shinyauthr::logoutUI("logout"))),
  title = "VMS FISH DATABASE",
  dashboardHeader(
    title = tags$div(
      tags$img(src = "logo.png", height = "40px", style = "margin-right: 1px;"),  # Add logo with spacing
      "FISH DATABASE"
    ),
    titleWidth = 300,
  ...
  dashboardSidebar(width = 150,
    ...
    sidebarMenu(id="tabs",style = "position: fixed; overflow: visible;",
      ...
      div(id="settings_menu", style = "position: fixed; overflow: visible;padding: 20px; bottom:10px",
          menuItem("Admin Settings", tabName = "admin_settings", icon = icon("cog")))%>% shinyjs::hidden()
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "admin_settings", uiOutput("settings_ui"))
...
server <- function(input, output, session) { 
  ...
  user_base <- fishdb$user_base %>%
    mutate(password_hash = sapply(password, sodium::password_store)) %>%
    select(-password)
  ...
  ##################################Deciding what elements are shown to normal vs admin users
  observeEvent(credentials()$info$permissions,{
    req(credentials()$user_auth)  # Ensures that the user is authenticated first
    if (!is.null(credentials()$info$permissions) && credentials()$info$permissions == "Administrator") {
      shinyjs::show("settings_menu")
      shinyjs::show("admin_map_upload")
    } else {
      shinyjs::hide("settings_menu")
    }
  })
  ...
  observeEvent(input$Fishroom_map_choices,{
    shinyjs::hide("show_stats")
  })
  ...
  observeEvent(input$fishroom_upload,{
    ...
    if(qc==T){
      ...
      room<-room_map()
      total_fish<- as.numeric(room$Number.of.fish)
  ...
  observeEvent(input$Add_nursery_confirm, {
    ...
    if(dup_check_archive==T){
      ...
      archive[nrow(archive)+1,]<-list(as.numeric(input$nTanks_add_n),as.numeric(input$Stock_no_add_n),format(input$DOB_add_n,"%d-%b-%Y"),input$Fish_name_add_n,
                                      input$Fish_Genotype_add_n,as.numeric(input$nFish_add_n),input$Fish_Responsible_add_n,
                                      input$Fish_fParent_add_n,input$Fish_MParent_add_n,as.numeric(input$Stock_no_fparent_add_n),as.numeric(input$Stock_no_Mparent_add_n),
                                      input$Experiment_n)
      
      log_data[nrow(log_data)+1,]<-list( format(Sys.time(), "%d-%b-%Y %H.%M"),as.numeric(input$Stock_no_add_n),format(input$DOB_add_n,"%d-%b-%Y"),input$Fish_name_add_n,
                                         input$Fish_Genotype_add_n,input$Fish_Responsible_add_n,as.numeric(input$nFish_add_n),
                                         paste0(toupper(user_info()$name)," ADDED STOCK IN THE ", input$Nursery_choices_add))
      zebra1[nrow(zebra1)+1,]<-list(input$nTanks_add_n,as.numeric(input$Stock_no_add_n),format(input$DOB_add_n,"%d-%b-%Y"),input$Fish_name_add_n,
                                    input$Fish_Genotype_add_n,input$Fish_Responsible_add_n,as.numeric(input$nFish_add_n),
                                    input$Experiment_n)
  ...
  observeEvent(input$Add_adult_confirm,{
    ...
    adult_file[locate_tank(adult_file,input$Tank_name_adult),]<-list(input$Tank_name_adult, as.numeric(input$Stock_no_add_adult), format(input$DOB_add_adult,"%d-%b-%Y"),
                                                                     input$Fish_name_add_adult, input$Fish_Genotype_add_adult, input$Fish_Responsible_add_adult,
                                                                     as.numeric(input$nFish_add_adult),paste(input$Experiment_label_adult,collapse = " , "),input$Food_label_adult,
                                                                     input$Notes_add_adult)
    log_data<-fishdb[["Database_logs"]]
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),as.numeric(input$Stock_no_add_adult),format(input$DOB_add_adult,"%d-%b-%Y"),
                                      input$Fish_name_add_adult,input$Fish_Genotype_add_adult,input$Fish_Responsible_add_adult,
                                      as.numeric(input$nFish_add_adult),
                                      paste0(toupper(user_info()$name)," ADDED STOCK IN THE ", adult_add_choice()))
    fishdb$Database_logs<-log_data
...
For Nursery fish transfers, select the stock number, and for Adult Fish transfers, select the tank location to choose the stock. You can update the fish number, genotype, experiment label, and food label for the new location. Once you submit the form, you will be prompted to verify your action. Finally, click the Confirm button to complete the transfer. Your actions will be logged. To transfer another fish, click the Reset button.
      ...
      tabsetPanel(
        tabPanel("From Nursery to Fish Rooms",
                 div(id="transfer fish_nursery",
                     h3(strong("TRANSFER FISH FROM NURSERY TO ADULT FISH ROOM")),
                     wellPanel(
                       useShinyjs(),
                       ...
                       div(id="nursery_transfer_upload_hide",
                           ...
                           div(id="nursery_transfer_dest",
                               h3(strong("TO :-")),
                               hr(),
                               selectizeInput(inputId = "nursery_choices_transfer_dest", label = "Select the Fish Room: ", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select Fish Room")),
                     ...
                     wellPanel(
                       ...
                       div(id="adult_transfer_between_upload_hide",
                           ...
                           div(id="adult_transfer_between_dest",
                               h3(strong("TO :-")),
                               hr(),
                               selectizeInput(inputId = "adult_choices_transfer_between_dest", label = "Select the Fish Room: ", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select Fish Room")),
  ...
  #######################Server side code for dynamically updating the fish room list before transfer###########################################################
  
  observeEvent(input$tabs,{
    if(input$tabs=="transfer_stocks"){
      fishdb<-readRDS("fishdatabase.rds")
      nursery_rooms<-fishdb[["nursery_info"]][,1]
      adult_rooms<-fishdb[["adult_info"]][,1]
      updateSelectizeInput(session, inputId = "nursery_choices_transfer", choices = nursery_rooms, server = TRUE)
      updateSelectizeInput(session, inputId = "nursery_choices_transfer_dest", choices = adult_rooms, server = TRUE)
      updateSelectizeInput(session, inputId = "adult_choices_transfer", choices = adult_rooms, server = TRUE)
      updateSelectizeInput(session, inputId = "adult_choices_between_transfer", choices = adult_rooms, server = TRUE)
      
    }
    
  })

  observeEvent(input$nursery_choices_transfer,{
  ...
  observeEvent(input$nursery_transfer_upload,{
    fishdb<-readRDS("fishdatabase.rds")
    room_loc<-locate_room(fishdb[["nursery_info"]], nursery_transfer_room())
    zebra_n<-fishdb[[fishdb[["nursery_info"]]$db_list_name[room_loc]]]
  ...
  observeEvent(input$nursery_transfer_select,{
    req(input$Select_nursery_stock_transfer)
  ...
  observeEvent(input$nursery_transfer_dest_select,{
    req(input$Select_nursery_stock_transfer)
    fishdb<-readRDS("fishdatabase.rds")
    n_room_loc<-locate_room(fishdb[["nursery_info"]], nursery_transfer_room())
    zebra_n<-fishdb[[fishdb[["nursery_info"]]$db_list_name[n_room_loc]]]
    a_room_loc<-locate_room(fishdb[["adult_info"]], nursery_transfer_dest_choice())
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    output$nursery_transfer_room_selection<- renderText({
              paste0("Your Fish will be transferred to the ",fishdb[["adult_info"]]$RoomName[a_room_loc]," !!")
          })
    ...
    fishn<- seq(1:(as.numeric(zebra_n[from_stock_loc,7])*2))
  ...
  observeEvent(input$nursery_Fish_transfer_submit,{
    ...
    zebra_n<-fishdb[[fishdb[["nursery_info"]]$db_list_name[n_room_loc]]]
    a_room_loc<-locate_room(fishdb[["adult_info"]], nursery_transfer_dest_choice())
    dest_room<-fishdb[["adult_info"]]$RoomName[a_room_loc]
  ...
  observeEvent(input$nursery_Fish_transfer_confirm,{
    ...
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    dest_room<-fishdb[["adult_info"]]$RoomName[a_room_loc]
    nursery_selected<-fishdb[["nursery_info"]]$db_list_name[n_room_loc]
    adult_selected<-fishdb[["adult_info"]]$db_list_name[a_room_loc]
    ...
    if (length(dest_loc>1)) {
      ...
      for (i in 1:length(dest_loc)){
        ...
        log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),input$Select_nursery_stock_transfer,as.character(zebra_n[from_stock_loc,3]),
                                          ...
                                          paste0(toupper(user_info()$name), " TRANSFERRED STOCK FROM ", nursery_transfer_room(), " TO ",dest_loc[i],
                                                 " IN ", dest_room))
          zebra_n[from_stock_loc,7]<- as.numeric(zebra_n[from_stock_loc,7])-as.numeric(input$nursery_transfer_n)


      }
      if(as.numeric(zebra_n[from_stock_loc,7])<=0){
      ...
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),input$Select_nursery_stock_transfer,as.character(zebra_n[from_stock_loc,3]),
                                        zebra_n[from_stock_loc,4],zebra_n[from_stock_loc,5],zebra_n[from_stock_loc,6],
                                        input$nursery_transfer_n,
                                        paste0(toupper(user_info()$name), " TRANSFERRED STOCK FROM ", nursery_transfer_room(), " TO ",input$Select_destination_tank_nursery_transfer,
                                               " IN ", dest_room))
      
      
      
      if(as.numeric(zebra_n[from_stock_loc,7])<=as.numeric(input$nursery_transfer_n)){
        zebra_n<-zebra_n[-from_stock_loc,]
      }else{
        zebra_n[from_stock_loc,7]<- as.numeric(zebra_n[from_stock_loc,7])-as.numeric(input$nursery_transfer_n)
      }
    ...
    fishdb[[nursery_selected]]<-zebra_n
    fishdb[[adult_selected]]<-zebra_ad
    fishdb$Database_logs<-log_data
    saveRDS(fishdb, "fishdatabase.rds")
  ...
  observeEvent(input$adultroom_transfer_select, {
    req(input$adult_stock_transfer_source_name)
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], adult_transfer_roomchoice())
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    ...
    if(qc==T){
      ...
      fishn<- seq(1:as.numeric(zebra_ad[adt_location,7]))
  ...
  observeEvent(input$adult_Fish_transfer_confirm,{
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra_ad[from_stock_loc,2], as.character(zebra_ad[from_stock_loc,3]),
                                      ...
                                      paste0(toupper(user_info()$name)," TRANSFERRED STOCK FROM ", adult_transfer_roomchoice()  ," TANK LOCATION- ", input$adult_stock_transfer_source_name, " TO ",input$adult_stock_transfer_destination_name
                                      ))
    ...
    updateSelectizeInput(session, inputId = "adult_stock_transfer_destination_name" ,choices = NULL,server= T,selected = NULL)
  ...
  #################################################################################################################################################
  #######Server side code for transferring fish between two adult room#############################################################################
  adult_transfer_between_source<- eventReactive(input$adultroom_between_transfer_upload,{input$adult_choices_between_transfer})
  observeEvent(input$adult_choices_between_transfer,{
    shinyjs::hide("adult_transfer_between_upload_hide")
    shinyjs::hide("adult_transfer_between_tanks")
    shinyjs::hide("adult_transfer_between_dest")
  })
  observeEvent(input$adultroom_between_transfer_upload,{
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], adult_transfer_between_source())
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    updateSelectizeInput(session, inputId = "adult_choices_transfer_between_dest", choices = NULL, server = TRUE)
    
    output$adult_stock_transfer_between_quickview<-DT::renderDataTable(
      zebra_ad, selection = "single",option=list(pageLength=10,lengthChange = TRUE,lengthMenu = c(5, 10, 20, 40, 80, 100))
    )
    
    output$adult_transfer_between_room_selection<- renderText({
      "" 
    })
    
    shinyjs::show("adult_transfer_between_upload_hide")
    shinyjs::hide("adult_transfer_between_tanks")
    shinyjs::hide("adult_transfer_between_dest")
    
    occupiedtanks_at<-find_occupied_tanks(zebra_ad)
    updateSelectizeInput(session, inputId = "adult_stock_transfer_between_source_name" ,choices = occupiedtanks_at,server= T,selected = NULL)
    
    observeEvent(input$adult_stock_transfer_between_quickview_rows_selected,{
      shinyjs::hide("adult_transfer_between_tanks")
      shinyjs::hide("adult_transfer_between_dest")
      selected_rows <- input$adult_stock_transfer_between_quickview_rows_selected
      selected_data <- zebra_ad[selected_rows, ]
      if(length(selected_rows)>0){
        updateSelectizeInput(session, inputId = "adult_stock_transfer_between_source_name" ,choices = occupiedtanks_at,server= T, selected =selected_data[1])
        output$selected_rows_adult_stock_transfer_between_output<- renderPrint(
          print(selected_data)
        )
      }
    })
  ...
  observeEvent(input$adultroom_transfer_between_select,{
    req(input$adult_stock_transfer_between_source_name)
    qc<- check_errors(input$adult_stock_transfer_between_source_name)
    shinyjs::hide("adult_transfer_between_tanks")
    if(qc==T){
      output$error_adultroom_transfer_between_select<-renderText(
        "ERROR !!! You must select a non-empty tank for transfer!!"
      )
      shinyjs::hide("adult_transfer_between_dest")
      ...
      shinyjs::show("adult_transfer_between_dest")
      adult_rooms_remaining<-setdiff(adult_rooms,adult_transfer_between_source())
      updateSelectizeInput(session, inputId = "adult_choices_transfer_between_dest", choices = adult_rooms_remaining, server = TRUE)
    }
  })
  
  adult_transfer_between_dest<- eventReactive(input$adult_transfer_between_dest_select,{input$adult_choices_transfer_between_dest})
  
  observeEvent(input$adult_transfer_between_dest_select,{
    req(input$adult_stock_transfer_between_source_name)
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], adult_transfer_between_dest())
    zebra_ad_dest<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    output$adult_transfer_between_room_selection<- renderText({
      paste0("Your Fish will be transferred to the ",fishdb[["adult_info"]]$RoomName[a_room_loc]," !!")
    })
    a_room_loc_source<- locate_room(fishdb[["adult_info"]], adult_transfer_between_source())
    zebra_ad_source<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc_source]]]

        shinyjs::show("adult_transfer_between_tanks")
        emptytanks_at<- find_empty_tanks(zebra_ad_dest)
        updateSelectizeInput(session, inputId = "adult_stock_transfer_between_destination_name" ,choices = emptytanks_at,server= T,selected = NULL)
        
        adt_location<- locate_tank(zebra_ad_source, input$adult_stock_transfer_between_source_name)
        fishn<- seq(1:as.numeric(zebra_ad_source[adt_location,7]))
        ov_geno<-orig_val(zebra_ad_source[adt_location,7],fishn)
        updateSelectizeInput(session, inputId = "adult_transfer_between_n",choices = fishn,selected = fishn[ov_geno], server=T)
        
        genotype_list<- unique(zebra_ad_source[,5])
        genotype_list<-append(genotype_list,"Enter genotype", after=0)
        ov_geno<-orig_val(zebra_ad_source[adt_location,5],genotype_list)
        updateSelectizeInput(session, inputId = "genotype_edit_adult_transfer_between",choices = genotype_list,selected = genotype_list[ov_geno], server=T)
        
        exp_label_std<-list("GENOTYPE","SURGERY","BREEDING STOCK","TRANSPLANT")
        exp_label<- unique(zebra_ad_source[,8])
        exp_label<-append(exp_label,exp_label_std,after = 0)
        ov_explab<- orig_val(zebra_ad_source[adt_location,8],exp_label)
        updateSelectizeInput(session, inputId = "Experiment_label_adult_transfer_between", selected = exp_label[ov_explab] ,choices = exp_label,server= T)
        
        food_label<-list("JA","JB","JC","JD","JE","JF","JG","JH","JI","JJ","SA","SB","SC","SD","SE","SF","SG","SH","SI","SJ",
                         "AA","AB","AC","AD","AE","AF","AG","AH","AI","AJ")
        ov_foodlab<- orig_val(zebra_ad_source[adt_location,9],food_label)
        updateSelectizeInput(session, inputId = "Food_label_adult_adult_transfer_between", selected = food_label[ov_foodlab],choices = food_label,server= T)
        
        notes_list<-unique(zebra_ad_source[,10]) 
        notes_list<-append(notes_list,"Enter notes", after = 0)
        ov_notes<- orig_val_notes(zebra_ad_source[adt_location, 10], notes_list)
        updateSelectizeInput(session, inputId = "Notes_adult_transfer_between", selected = notes_list[ov_notes], choices = notes_list, server = T)
        
        output$selected_rows_adult_stock_transfer_between_output<- renderPrint(
          print(zebra_ad_source[adt_location,])
        )
    # output$adult_transfer_between_room_selection<- renderText({
    #   "" 
    # })
  ...
  observeEvent(input$adult_stock_transfer_between_source_submit,{
    req(input$adult_stock_transfer_between_source_name)
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc_source<- locate_room(fishdb[["adult_info"]], adult_transfer_between_source())
    zebra_ad_source<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc_source]]]
    
    from_stock_loc <- locate_tank(zebra_ad_source, input$adult_stock_transfer_between_source_name)
    user_input<-c(input$adult_stock_transfer_between_source_name, input$adult_stock_transfer_between_destination_name, input$adult_transfer_between_n,
                  input$genotype_edit_adult_transfer_between, paste(input$Experiment_label_adult_transfer_between, collapse = " , "), input$Food_label_adult_adult_transfer_between,
                  input$Notes_adult_transfer_between
    )
    
    qc<-check_errors(user_input)
    if(qc==T){
      output$error_transfer_between_Adultfish<- renderText(
        "ERROR !!! Please fill all the mandatory fields"
      ...
      showModal(modalDialog(
        tagList(
          paste0("Please confirm the transfer of ",input$adult_transfer_between_n, " fish of " ,zebra_ad_source[from_stock_loc,2], " - ", zebra_ad_source[from_stock_loc,4] ),
          paste0(" from ", input$adult_stock_transfer_between_source_name, " to the tank location ", input$adult_stock_transfer_between_destination_name,
                 " in the ", input$adult_choices_transfer_between_dest, ".")
          
        ),
        title = strong("Confirm Transfer"),
        footer = tagList(actionButton("adult_Fish_transfer_between_confirm","Confirm"), modalButton("Cancel"))
      ))
  ...
  observeEvent(input$adult_Fish_transfer_between_confirm,{
    req(input$adult_stock_transfer_between_source_name)
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc_source<- locate_room(fishdb[["adult_info"]], adult_transfer_between_source())
    zebra_ad_source<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc_source]]]
    a_room_loc<-locate_room(fishdb[["adult_info"]], adult_transfer_between_dest())
    zebra_ad_dest<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    adult_dest_selected<-fishdb[["adult_info"]]$db_list_name[a_room_loc]
    adult_dest_source<-fishdb[["adult_info"]]$db_list_name[a_room_loc_source]
    
    
    from_stock_loc <- locate_tank(zebra_ad_source, input$adult_stock_transfer_between_source_name)
    to_stock_loc<- locate_tank(zebra_ad_dest, input$adult_stock_transfer_between_destination_name)
    
    zebra_ad_dest[to_stock_loc, ]<-list(
      input$adult_stock_transfer_between_destination_name, zebra_ad_source[from_stock_loc,2],  as.character(zebra_ad_source[from_stock_loc,3]), zebra_ad_source[from_stock_loc,4],
      input$genotype_edit_adult_transfer_between, zebra_ad_source[from_stock_loc,6], input$adult_transfer_between_n, paste(input$Experiment_label_adult_transfer_between, collapse = " , "),
      input$Food_label_adult_adult_transfer_between, input$Notes_adult_transfer_between) 
    
    
    log_data<-fishdb[["Database_logs"]]
    
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra_ad_source[from_stock_loc,2], as.character(zebra_ad_source[from_stock_loc,3]),
                                      zebra_ad_source[from_stock_loc,4],input$genotype_edit_adult_transfer_between, zebra_ad_source[from_stock_loc,6],
                                      input$adult_transfer_between_n,
                                      paste0(toupper(user_info()$name), " TRANSFERRED STOCK FROM ", adult_transfer_between_source()  ," TANK LOCATION- ", input$adult_stock_transfer_between_source_name, " TO ",input$adult_stock_transfer_between_destination_name,
                                             " IN ", adult_transfer_between_dest()
                                      ))
    
    fishdb$Database_logs<-log_data
    
    if(as.numeric(zebra_ad_source[from_stock_loc,7])<=as.numeric(input$adult_transfer_between_n)){
      zebra_ad_source[from_stock_loc,2:10]<-NA
    }else{
      zebra_ad_source[from_stock_loc,7]<- as.numeric(zebra_ad_source[from_stock_loc,7])-as.numeric(input$adult_transfer_between_n)
    }
    
    fishdb[[adult_dest_selected]]<-zebra_ad_dest
    fishdb[[adult_dest_source]]<-zebra_ad_source
    saveRDS(fishdb, "fishdatabase.rds")
    ...
    showModal(modalDialog(
      ...
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("adult_transfer_between_modal_success","Close"))
    ))
  ...
  observeEvent(input$adult_transfer_between_modal_success,{
    #reset("transfer fish_nursery")
    shinyjs::hide("adult_transfer_between_dest")
    shinyjs::hide("adult_transfer_between_tanks")
    shinyjs::hide("adult_transfer_between_upload_hide")
  ...
  observeEvent(input$reset_adult_transfer_between,{
    reset("Between Fish Rooms")
    shinyjs::hide("adult_transfer_between_dest")
    shinyjs::hide("adult_transfer_between_tanks")
    shinyjs::hide("adult_transfer_between_upload_hide")
    output$adult_transfer_room_selection<- renderText({
  ...
  observeEvent(input$reset_adult_transfer_between,{
    output$adult_fish_transfer_between_success<- renderText({
      paste0("Your form is been reset. Please submit again to transfer stocks!!")
    })
  })
...
If you click the Euthanize button, all the fish associated with the selected stock number or tank location will be removed from the fish room map. Separate tabs are provided for individual tank edits and bulk edits.
      ...
      tabsetPanel(
        tabPanel("Individual Tank Editing",
                 h3(strong("EDIT TANKS INDIVIDUALLY")),
                 wellPanel(
                   useShinyjs(),
...
#########################################################Dynamically updating the bulk edit fish room maps################################################
  observeEvent(input$tabs,{
    if(input$tabs=="edit_stocks"){
      fishdb<-readRDS("fishdatabase.rds")
      rooms<-c(fishdb[["nursery_info"]][,1],fishdb[["adult_info"]][,1])
      updateSelectizeInput(session, inputId = "edit_fish_room_choices", choices = rooms, server = TRUE)
      updateSelectizeInput(session, inputId = "bulk_edit_fish_room_choices", choices = rooms, server = TRUE)

    }
  })
  ################################################################################################################################################
  bulk_edit_fishroom_choice<-eventReactive(input$bulk_edit_fish_room_upload,{input$bulk_edit_fish_room_choices})
  observeEvent(input$bulk_edit_fish_room_choices,{
  ...
  bulk_edit_file<-eventReactive(input$bulk_edit_fish_room_upload,{
    fishdb<-readRDS("fishdatabase.rds")
    combined_info<-rbind(fishdb[["nursery_info"]],fishdb[["adult_info"]])
    room_loc<-locate_room(combined_info, bulk_edit_fishroom_choice())
    zebra<-fishdb[[combined_info$db_list_name[room_loc]]]
    return(zebra)
  ...
  bulk_edit_file_name<-eventReactive(input$bulk_edit_fish_room_upload,{
    ...
    combined_info<-rbind(fishdb[["nursery_info"]],fishdb[["adult_info"]])
    room_loc<-locate_room(combined_info, bulk_edit_fishroom_choice())
    room_selected<-combined_info$db_list_name[room_loc]
  ...
  observeEvent(input$bulk_edit_fish_room_upload,{
    ...
    observeEvent(input$bulk_edit_fish_room_quickview_rows_selected,{
      ...
      if(length(selected_rows)>0){
          if(is.element(bulk_edit_fishroom_choice(),adult_rooms)){
        ...
        }else if(is.element(bulk_edit_fishroom_choice(),nursery_rooms)){
  ...
  observeEvent(input$bulkedit_Resp,{
    req(input$bulk_edit_fish_room_quickview_rows_selected)
  ...
  observeEvent(input$AFish_bulkedit_resp_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        ...
                                        paste0(toupper(user_info()$name), " EDITED THE RESPONSIBLE PERSON NAME "))
    ...
    fishdb$Database_logs<-log_data
    fishdb[[bulk_edit_file_name()]]<-data
    saveRDS(fishdb, "fishdatabase.rds")
  ...
  observeEvent(input$AFish_bulkedit_explabel_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        ...
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name)," EDITED THE EXPERIMENT LABEL TO ", input$bulk_edit_explabel_Ad_choice))
  ...
  observeEvent(input$AFish_bulkedit_foodlabel_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        bulk_edit_file()[selected_rows[i],4],bulk_edit_file()[selected_rows[i],5], bulk_edit_file()[selected_rows[i],6],
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name), " EDITED THE FOOD LABEL TO ", input$bulk_edit_foodlabel_Ad_choice))
  ...
  observeEvent(input$AFish_bulkedit_notes_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        bulk_edit_file()[selected_rows[i],4],bulk_edit_file()[selected_rows[i],5], bulk_edit_file()[selected_rows[i],6],
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name)," EDITED THE NOTES TO ", input$bulk_edit_notes_Ad_choice))
  ...
  observeEvent(input$bulk_AFish_euthanize_all_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        bulk_edit_file()[selected_rows[i],4],bulk_edit_file()[selected_rows[i],5], bulk_edit_file()[selected_rows[i],6],
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name)," EUTHANIZED THE STOCK "))
  ...
  observeEvent(input$NFish_bulkedit_notes_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        bulk_edit_file()[selected_rows[i],4],bulk_edit_file()[selected_rows[i],5], bulk_edit_file()[selected_rows[i],6],
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name)," EDITED THE NOTES to ", input$bulk_edit_notes_N_choice))
  ...
  observeEvent(input$bulk_NFish_euthanize_all_confirm,{
    ...
    for (i in 1:length(selected_rows)) {
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),bulk_edit_file()[selected_rows[i],2], as.character(bulk_edit_file()[selected_rows[i],3]),
                                        bulk_edit_file()[selected_rows[i],4],bulk_edit_file()[selected_rows[i],5], bulk_edit_file()[selected_rows[i],6],
                                        bulk_edit_file()[selected_rows[i],7],
                                        paste0(toupper(user_info()$name)," EUTHANIZED THE STOCK FROM NURSERY "))
  ...
  observeEvent(input$edit_fish_room_upload,{
    ...
    }else if (is.element(input$edit_fish_room_choices,nursery_rooms)){
  ...
  observeEvent(input$edit_fish_room_choices,{
    shinyjs::hide("edit_nursery_room")
    shinyjs::hide("edit_adult_room")
  ...
  observeEvent(input$edit_Afish,{
    ...
    if(qc==T){
      ...
      fishn<- seq(1:(as.numeric(zebra_ad[adt_location,7])*2))
      ov_geno<-orig_val(zebra_ad[adt_location,7],fishn)
  ...
  observeEvent(input$AFish_edit_confirm,{
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra_ad[adt_location,2], as.character(zebra_ad[adt_location,3]),
                                      ...
                                      paste0(toupper(user_info()$name)," EDITED THE ADULT STOCK ")
    )
    ###saving all changes
  ...
  observeEvent(input$Euthanise_AFish,{
    ...
    if(qc==T){
      output$error_edit_Afish_select<-renderText(
        "ERROR !!! There is nothing selected to Euthanize!!"
      )
    }else{
  ...
  observeEvent(input$AFish_euthanize_all_confirm,{
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra_ad[adt_location,2], as.character(zebra_ad[adt_location,3]),
                                      ...
                                      paste0(toupper(user_info()$name)," EUTHANIZED THE ADULT STOCK")
    )
  ...
  observeEvent(input$edit_Nfish,{
    ...
    ntanks<- seq(1:(as.numeric(zebra[Nstock_loc,1])))
    ...
    fishn<- seq(1:(as.numeric(zebra[Nstock_loc,7])*2))
  ...
  observeEvent(input$NFish_edit_confirm,{
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra[Nstock_loc,2], as.character(zebra[Nstock_loc,3]),
                                      ...
                                      paste0(toupper(user_info()$name)," EDITED THE NURSERY STOCK "))
    
    ###saving all changes
    fishdb$Database_logs<-log_data
    fishdb[[nursery_selected]]<-zebra
    saveRDS(fishdb, "fishdatabase.rds")
  ...
  observeEvent(input$NFish_euthanize_all_confirm,{
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),zebra[Nstock_loc,2], as.character(zebra[Nstock_loc,3]),
                                      ...
                                      zebra[Nstock_loc,7],
                                      paste0(toupper(user_info()$name)," EUTHANIZED THE NURSERY STOCK")
    )
  ...
  output$archive_ui<- renderUI({
    ...
    fluidPage(
      fluidRow(
        ...
        wellPanel(
          useShinyjs(),
          h2(strong("FISH DATABASE ARCHIVE")),
          hr(),
          p(style = "text-align: justify", "
            Here, you will be able to visualize all fish stocks that are/were present in this fish facility. 
...
Here, you can identify the lineage tree of any stock in the archive. First, upload the archive file by clicking the Upload button. This will display a quick view of the fish stock archive. Use the search box in the top-left corner to locate your stock number. Then, enter the desired stock number in the field below the archive table and click Identify Genealogy. 
          ...
          div(id="genealogy_tree_hide",
              ...
              h4(style=  "color:red",textOutput("genealogy_error")),
  ...
  observeEvent(input$Genealogy_generate,{
    req(input$Select_genealogy_stock)
    ...
    output$Download_genealogy_tree<-downloadHandler(
      ...
      content=function(file){
        pdf(file, width = 8, height = 8,useDingbats = FALSE)  # Open a PDF device
        plot(g, layout = co, edge.arrow.size = 1, edge.color = "black", edge.width = 1.5,
             vertex.size = 25, vertex.frame.color = "black", vertex.label.color = "black",
             vertex.color = "orange", vertex.label.cex = 1)
...
Once you finalize the label details, click Create Label. Finally, click the Download Label button to save the label to your computer, which you can then print using your own printer.")),
      ...
      div(id="print_label_map",
          fluidRow(
            wellPanel(
              strong(h3("SELECT FISH STOCK TO CREATE LABEL")),
              hr(),
      ...
      div(id="print_label",
          ...
          wellPanel(
            ...
            hr(),
            fluidRow(column(width = 2,actionButton(inputId = "preview_label_button", label="Create Label")),
  ...
  ########################################################################################################################################################
  observeEvent(input$tabs,{
    if(input$tabs=="print_label"){
      shinyjs::hide("print_label")
      shinyjs::hide("print_label_map")
      fishdb<-readRDS("fishdatabase.rds")
      rooms<-c(fishdb[["nursery_info"]][,1],fishdb[["adult_info"]][,1])
      updateSelectizeInput(session, inputId = "print_label_Fishroom_choices", choices = rooms, server = TRUE)
    }
    
  })
  
  observeEvent(input$print_label_Fishroom_choices,{
    shinyjs::hide("print_label")
    shinyjs::hide("print_label_map")
  })
  ...
  print_label_room_map<-eventReactive(input$print_label_fishroom_upload,{
    ...
    }else if(is.element(print_label_fishroom_choice(),nursery_rooms)){
      n_room_loc<-locate_room(fishdb[["nursery_info"]], print_label_fishroom_choice())
      zebra<-fishdb[[fishdb[["nursery_info"]]$db_list_name[n_room_loc]]]
  ...
  observeEvent(input$print_label_fishroom_upload,{
    ...
    output$print_label_fishroom_map<-DT::renderDataTable(
      print_label_room_map(),selection = "single",option=list(pageLength=10,lengthChange = TRUE,lengthMenu = c(5, 10, 20, 40, 80, 100) )
    )
    shinyjs::show("print_label_map")
...
Once the upload is complete, click the Replace Fish Map button to finalize the changes.")),
        ...
        uiOutput("image_ui"),
        hr(),
        div(id="admin_map_upload",
            useShinyjs(),
            fileInput("fishroom_map_image", "Upload a map of your Fish Facility (Max size: 500KB)", 
                      accept = c('image/png', 'image/jpeg', 'image/jpg')),
            fluidRow(actionButton(input="replace_fish_map",label = "REPLACE FISH MAP"))
            ,
            em("(Note: If the uploaded image don't show up, please restart the web browser and clear cache memory.)"),
            h4(style=  "color:red",textOutput("file_warning"))
            )%>% shinyjs::hidden()
      ),
      wellPanel(
   ...
   Thank you for using it."
      ...
      wellPanel(
        ...
        tags$ul(
          tags$li("110224: Removed all restriction to transfer fish.Added function to transfer fish between adult rooms."),
          tags$li("110324: Removed all .csv files for storing database info. Update the whole code to read from rds files,
                  which will enable dynamic reading of fish rooms.")
        ),
        p(strong(tags$u("v1.4.0"))),
        tags$ul(
          tags$li("110924: Added admin settings which will manage fish rooms, tanks, accounts and passwords.
                  These functionalities can only be accessed through an admin user account."),
          tags$li("111424: Added fulldata backup option under the clean slate menu."),
          tags$li("111524: Added admin feature to restore the fishdatbase info from a previously backed up rds file.")
  ...
  observeEvent(input$replace_fish_map,{
    req(input$fishroom_map_image)
    file_size <- input$fishroom_map_image$size / 1024
    
    if (file_size > 500) {
      output$file_warning <- renderText({
        paste0( "File is too large. Please upload an image smaller than 500KB.")
        })
    } else {
      output$file_warning <- renderText("") 
      save_path <- file.path("WWW", "fish_facility_map.png")
      file.copy(from = input$fishroom_map_image$datapath, to = save_path, overwrite = TRUE)
      showModal(modalDialog(
        tagList(
          strong(
            paste0("You successfully replaced the fish facility map. Restart fish database to see the changes !!")
          )
        ),
        title = p(style= "color:green",strong("SUCCESS !! ")),
        footer = tagList(actionButton("replace_fish_map_success","Close"))
      ))
      
    }
    output$image_ui <- renderUI({
      tags$img(src = paste0("fish_facility_map.png"), height="100%", width="100%", alt="Fish_facility_map")
    })
  })
  output$image_ui <- renderUI({
    tags$img(src = paste0("fish_facility_map.png"), height="100%", width="100%", alt="Fish_facility_map")
  ...
  observeEvent(input$replace_fish_map_success,{
    removeModal()
  })
  #################################################################Admin Settings Menu##########################################################
...
It is strongly advised to download the respective file and create a backup before removing any tanks."
                       ...
                       actionButton(inputId = "set_edit_tanks_upload", label="Upload"),
                       hr(),
                       div(id="set_edit_tanks_table",
                           fluidRow(column(width=12, dataTableOutput("set_edit_tanks_quickview"),
                                               style = "width: 100%; overflow-x: auto;")),
                           hr(),
                           fluidRow(column(width = 2,actionButton(inputId = "set_add_tanks", label="Add Tanks")),
                                    column(width = 2,actionButton(inputId = "set_delete_tanks", label="Delete Tanks"))
                                          
                            
                           ),
                           h4(style=  "color:red",textOutput("set_delete_tanks_error")),
                           hr(),
                           div(id="add_tanks_name",
                               h3(strong("ADD NEW TANKS")),
                               hr(),
                               fluidRow(column(width = 2, textInput(inputId = "tank_prefix", label = "Tank Name Prefix* : ", value = "TANK")),
                                        column(width = 2, offset = 2, numericInput(inputId = "add_tank_counter",label = "Starting counter No.*: ", value = 1,min = 1)),
                                        column(width = 2, offset = 2, numericInput(inputId = "add_tank_n",label = "Number of tanks*: ", value = 10,min = 1))
                                        ),
                               actionButton(inputId = "set_add_tanks_submit", label="Submit"),
                               h4(style=  "color:red",textOutput("set_add_tanks_error")),
                               )%>% shinyjs::hidden()
                       )%>% shinyjs::hidden()
...
To begin, please click one of the buttons below."
                       ...
                       fluidRow(column(width = 2,actionButton(inputId = "create_new_room", label="CREATE")),
                                column(width = 2,actionButton(inputId = "remove_fish_room", label="REMOVE")),
                                column(width = 2,actionButton(inputId = "rename_fish_room", label="RENAME"))
                                
                                
                       )
                       
                     ),
                     div(id="set_create_room",
                         wellPanel(
                           h3(strong("CREATE NEW FISH ROOMS")),
                           hr(),
                           textInput(inputId = "new_room_name", label = "Fish room name* : ", value = "New Fish Room"),
                           radioButtons(inputId = "new_room_format", label = "Select Fish Room Type: ", choices = c("Nursery Fish Room", "Adult Fish Room")),
                           actionButton(inputId = "new_room_submit", label="Submit"),
                           h4(style=  "color:red",textOutput("set_create_room_error"))
                         )
                     )%>% shinyjs::hidden(),
                     div(id="set_remove_room",
                         wellPanel(
                           h3(strong("REMOVE FISH ROOMS")),
                           hr(),
                           selectizeInput(inputId = "remove_room_list", label = "Select the Fish Room: ", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select Fish Room")),
                           actionButton(inputId = "remove_room_submit", label="Submit"),
                           h4(style=  "color:red",textOutput("set_remove_room_error"))
                         )
                         
                     )%>% shinyjs::hidden(),
                     div(id="set_rename_room",
                         wellPanel(
                           h3(strong("RENAME FISH ROOMS")),
                           hr(),
                           selectizeInput(inputId = "rename_room_list", label = "Select the Fish Room: ", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select Fish Room")),
                           textInput(inputId = "rename_room_name", label = "New name for the Room* : ", value = "New Name"),
                           actionButton(inputId = "rename_room_submit", label="Submit"),
                           h4(style=  "color:red",textOutput("set_rename_room_error"))
                         )
                         
                     )%>% shinyjs::hidden()
                 )),
        tabPanel("Edit Accounts & Passwords",
                 div(id="set_ac_pwd",
                     h3(strong("EDIT ACCOUNTS & PASSWORDS")),
                     wellPanel(
                       useShinyjs(),
                       p(style=  "color:green",strong("Here, you will be able change passwords and add/remove user accounts. Click one of the 
                                                      buttons below to begin with changes."
                       )),
                       hr(),
                       fluidRow(column(width = 2,actionButton(inputId = "add_new_user", label="ADD NEW USER")),
                                column(width = 2,offset = 1,actionButton(inputId = "remove_user", label="REMOVE USER")),
                                column(width = 2,offset = 1,actionButton(inputId = "change_pwd", label="CHANGE PASSWORD"))
                       )
                     ),
                     div(id="set_new_user",
                         wellPanel(
                           h3(strong("ADD A NEW USER")),
                           hr(),
                           textInput(inputId = "new_user_name", label = "New User Name* : ", value = "New User"),
                           radioButtons(inputId = "new_user_permission", label = "Select User Type: ", choices = c("Standard", "Administrator")),
                           passwordInput(inputId = "new_pwd", label = "Enter password* :"),
                           passwordInput(inputId = "new_pwd_confirm", label = "Re enter password* :"),
                           actionButton(inputId = "new_user_submit", label="Add User"),
                           h4(style=  "color:red",textOutput("set_new_user_error"))
                         )
                     )%>% shinyjs::hidden(),
                     div(id="set_remove_user",
                         wellPanel(
                           h3(strong("REMOVE USER")),
                           hr(),
                           selectizeInput(inputId = "delete_user", label = "Select User to Delete :", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select User")),
                           actionButton(inputId = "delete_user_submit", label="Delete User"),
                           h4(style=  "color:red",textOutput("set_delete_user_error"))
                         )
                     )%>% shinyjs::hidden(),
                     div(id="set_change_pwd",
                         wellPanel(
                           h3(strong("CHANGE PASSWORD")),
                           hr(),
                           selectizeInput(inputId = "change_pwd_user", label = "Select User to Change Password :", choices= NULL ,selected=NULL, multiple = F,options = list(create=F, placeholder = "Select User")),
                           passwordInput(inputId = "change_pwd", label = "Enter password* :"),
                           passwordInput(inputId = "change_pwd_confirm", label = "Re enter password* :"),
                           actionButton(inputId = "change_pwd_submit", label="Change Password"),
                           h4(style=  "color:red",textOutput("set_change_pwd_error"))
                         )
                     )%>% shinyjs::hidden(),
                 )),
        tabPanel("Clean Slate Protocol",
                 div(id="clean_slate",
  ...
  Select the files you wish to delete and click the CLEAN SLATE button to proceed. Before performing the CLEAN SLATE PROTOCOL, we strongly advise you to create a full database backup by clicking the CREATE A FULL DATABASE BACKUP button. This will download an .rds file containing all the information related to your database.
                       ...
                       downloadButton(
                         outputId  = "create_backup",
                         label = "CREATE A FULL DATABASE BACKUP",
                         style = "color: #fff;background-color: #5cb85c; border-color: #4cae4c"
                       ),
                       br(),br(),
                       actionButton(
                         inputId = "clean_slate_submit",
                         label = "CLEAN SLATE",
                         style = "color: #fff; background-color: #d9534f; border-color: #d43f3a"
                       ),
                      
                       h4(style=  "color:red",textOutput("clean_state_error"))
                       
                       
                     )
                 )),
        tabPanel("Restore Database",
                 div(id="restore_db",
...
First, click Browse to select the .rds file saved on your computer. Once the file is uploaded, click the RESTORE DATABASE FILE button to complete the restoration."
                       ...
                       actionButton(
                         inputId = "restore_rds",
                         label = "RESTORE DATABASE FILE (.rds)",
                         style = "color: #fff; background-color: #5cb85c; border-color: #4cae4c"
                       ),
                       h4(style=  "color:red",textOutput("restore_rds_error")),
                     ))
                 )
      ))
  ...
  ####################################################Server side code for Admin Settings##########################################################
  #################################################################################################################################################
 #dynamically updating the input lists
  observeEvent(input$tabs,{
    if(input$tabs=="admin_settings"){
      fishdb<-readRDS("fishdatabase.rds")
      user_base<-fishdb$user_base
      updateSelectizeInput(session, inputId = "set_edit_tanks", choices = fishdb[["adult_info"]][,1], server = TRUE)
      updateSelectizeInput(session, inputId = "delete_user", choices = setdiff( user_base$user,"Administrator"), server = TRUE)
      updateSelectizeInput(session, inputId = "change_pwd_user", choices = user_base$user, server = TRUE)
      shinyjs::show("admin_map_upload")
  ...
  observeEvent(input$set_edit_tanks,{
    shinyjs::hide("set_edit_tanks_table")
    shinyjs::hide("add_tanks_name")
  })
  observeEvent(input$set_edit_tanks_upload,{
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], input$set_edit_tanks)
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    output$set_edit_tanks_quickview<-DT::renderDataTable(
      zebra_ad, selection = "multiple",option=list(pageLength=10,lengthChange = TRUE,lengthMenu = c(5, 10, 20, 40, 80, 100))
    )
    shinyjs::show("set_edit_tanks_table")
    shinyjs::hide("add_tanks_name")
  })
  

  ######adding tank to fish room#######################################
  observeEvent(input$set_add_tanks,{
    shinyjs::show("add_tanks_name")
  })
  observeEvent(input$set_add_tanks_submit,{
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], input$set_edit_tanks)
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    tanks_old<-zebra_ad[,1]
    user_input<-c(input$tank_prefix,input$add_tank_n, input$add_tank_counter)
    qc<-check_errors(user_input)
    if(qc==T){
      output$set_add_tanks_error<- renderText(
        "ERROR !!! Please fill all the *mandatory fields."
      ...
      newtanks<-c(paste0(input$tank_prefix, input$add_tank_counter:(input$add_tank_n+input$add_tank_counter-1)))
      dup=F
      for (i in newtanks) {
        dup_check_tankname<- dup_check(i,tanks_old)
        if(dup_check_tankname==T){
          dup=T
          duped_tank<-i
        }
      }
      if(dup==T){
        output$set_add_tanks_error<- renderText(
          paste0("ERROR !!! One or more of your tank name/s ", "(e.g. ",duped_tank, ") "," is/are already present in this room. ", "Change you prefix." )
        ...
        showModal(modalDialog(
          tagList(
            paste0("Please confirm adding following tanks to the ",input$set_edit_tanks, " :- "),
            paste0(paste(newtanks, collapse = " , "))
          ),
          title = strong("CONFIRM"),
          footer = tagList(actionButton("set_add_tanks_confirm","Confirm"), modalButton("Cancel"))
        ))
  ...
  observeEvent(input$set_add_tanks_confirm,{
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], input$set_edit_tanks)
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    adult_selected<-fishdb[["adult_info"]]$db_list_name[a_room_loc]
    newtanks<-c(paste0(input$tank_prefix, input$add_tank_counter:(input$add_tank_n+input$add_tank_counter-1)))
    tank_length<-length(zebra_ad[,1])
    for (i in 1:length(newtanks)) {
      zebra_ad[tank_length+i,1]<-newtanks[i]
    }
    log_data<-fishdb[["Database_logs"]]
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," ADDED TANKS TO THE ", input$set_edit_tanks))
    fishdb$Database_logs<-log_data
    fishdb[[adult_selected]]<-zebra_ad
    ...
    showModal(modalDialog(
      tagList(
        ...
        paste0("Your tanks are successfully added !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("set_add_tanks_modal_success","Close"))
    ))
    
  })
  
  observeEvent(input$set_add_tanks_modal_success,{
    shinyjs::hide("set_edit_tanks_table")
    shinyjs::hide("add_tanks_name")
    removeModal()
  })
  
  ###########Deleting tanks in the fish room####################################################################################################
  delete_tanks_room<-eventReactive(input$set_delete_tanks,{
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], input$set_edit_tanks)
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    return(zebra_ad)
  ...
  observeEvent(input$set_delete_tanks,{
    selected_rows <- input$set_edit_tanks_quickview_rows_selected
    deleting_tanks<-delete_tanks_room()[selected_rows, 1]
    if(length(selected_rows)>0){
      output$set_delete_tanks_error<- renderText(
        paste0("" )
      ...
      for( i in 1:length(selected_rows)){
        showModal(modalDialog(
          tagList(
            paste0("Please confirm removing following tank/s from the ",input$set_edit_tanks," :- "),
            br(),
            paste0(paste(deleting_tanks, collapse = " , "))
          ),
          title = p(style="color:red",strong("REMOVE TANKS")),
          footer = tagList(actionButton("remove_tanks_confirm","Confirm"), modalButton("Cancel"))
        ))
      }

    }else{
      output$set_delete_tanks_error<- renderText(
        paste0("ERROR !!! You didn't select any tanks to remove" )
        
      )
    }
  })
  
  observeEvent(input$remove_tanks_confirm,{
    req(input$set_edit_tanks_quickview_rows_selected)
    fishdb<-readRDS("fishdatabase.rds")
    a_room_loc<-locate_room(fishdb[["adult_info"]], input$set_edit_tanks)
    zebra_ad<-fishdb[[fishdb[["adult_info"]]$db_list_name[a_room_loc]]]
    adult_selected<-fishdb[["adult_info"]]$db_list_name[a_room_loc]
    ...
    for( i in 1:length(selected_rows)){
      selected_rows<-sort(selected_rows, decreasing = T)
      zebra_ad<-zebra_ad[-selected_rows[i],]
    }
    log_data<-fishdb[["Database_logs"]]
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," REMOVED TANKS FROM THE ", input$set_edit_tanks))
    fishdb$Database_logs<-log_data
    fishdb[[adult_selected]]<-zebra_ad
    saveRDS(fishdb, "fishdatabase.rds")
    removeModal()
    showModal(modalDialog(
      tagList(
        ...
        paste0("Your have successfully removed tanks !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("remove_tanks_modal_success","Close"))
    ))
  })
  
  
  observeEvent(input$remove_tanks_modal_success,{
    shinyjs::hide("set_edit_tanks_table")
    shinyjs::hide("add_tanks_name")
    removeModal()
  })
  
  ##############################################################Server side code for creating new Fish room#############################################
  observeEvent(input$create_new_room,{
    shinyjs::show("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::hide("set_rename_room")
  })
  ...
  observeEvent(input$new_room_submit,{
    ...
    if(qc==T){
      output$set_create_room_error<- renderText(
        "ERROR !!! Please fill all the *mandatory fields."
        
      )
    }else if(check_name_dup==T){
      output$set_create_room_error<- renderText(
        "ERROR !!! This name already exist for one of the rooms."
      ...
      showModal(modalDialog(
        tagList(
          paste0("Please confirm adding the room ",input$new_room_name, " to the fish database")
        ),
        title = strong("CONFIRM"),
        footer = tagList(actionButton("set_create_room_confirm","Confirm"), modalButton("Cancel"))
      ))
    }
    
  })
  
  observeEvent(input$set_create_room_confirm,{
    fishdb<-readRDS("fishdatabase.rds")
    if(input$new_room_format=="Nursery Fish Room"){
      db_list_name<-paste0("vms_",gsub(" ","",input$new_room_name))
      fishdb[["nursery_info"]]<-rbind(fishdb[["nursery_info"]],c(input$new_room_name, db_list_name,"Yes"))
      col_names<- c("No..of.Tanks" ,  "Stock.number." , "Date.of.Birth" , "Fish.name" ,     "Genotype." ,     "Responsible" ,   "Number.of.fish", "Notes" )
      df<-data.frame(matrix(nrow = 0, ncol = length(col_names)))
      colnames(df) <- col_names
      fishdb[[db_list_name]]<-df
      
    }
    if(input$new_room_format=="Adult Fish Room"){
      db_list_name<-paste0("vms_",gsub(" ","",input$new_room_name))
      fishdb[["adult_info"]]<-rbind(fishdb[["adult_info"]],c(input$new_room_name, db_list_name,"Yes"))
      col_names<-c("Tank.Name" ,       "Stock.number." ,   "Date.of.Birth" ,   "Fish.name",        "Genotype."  ,      "Responsible" ,     "Number.of.fish",   "Experiment.label",
                   "Food.label" ,      "Notes"  )
      df<-data.frame(matrix(nrow = 10, ncol = length(col_names)))
      colnames(df) <- col_names
      df[,1]<-paste0("TANK",1:10)
      fishdb[[db_list_name]]<-df
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," ADDED THE FISH ROOM ", toupper(input$new_room_name), " TO THIS
                                             DATABASE"))
    fishdb$Database_logs<-log_data
    saveRDS(fishdb, "fishdatabase.rds")
    ...
    showModal(modalDialog(
      tagList(
        ...
        paste0("The new fish room is successfully added to this database !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("set_create_room_success","Close"))
    ))
    
  })
  
  observeEvent(input$set_create_room_success,{
    shinyjs::hide("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::hide("set_rename_room")
    removeModal()
  })
  #########################################################Server side code for removing fish room from the database##############################
  observeEvent(input$remove_fish_room,{
    fishdb<-readRDS("fishdatabase.rds")
    rooms<-c(fishdb[["nursery_info"]][,1],fishdb[["adult_info"]][,1])
    updateSelectizeInput(session, inputId = "remove_room_list", choices = rooms, server = TRUE)
    shinyjs::hide("set_create_room")
    shinyjs::show("set_remove_room")
    shinyjs::hide("set_rename_room")
  })
  
  observeEvent(input$remove_room_submit,{
    req(input$remove_room_list)
    output$set_remove_room_error<- renderText(
      ""
    )
    showModal(modalDialog(
      tagList(
        paste0("Please confirm removing the ",input$remove_room_list, " from this fish database."),
        paste0(" Note that this action cannot be undone.")
      ),
      title = tags$strong(tags$span("CAUTION !!! CONFIRM REMOVING FISH ROOM", style = "color: red;")),
      footer = tagList(actionButton("set_delete_room_confirm","Confirm"), modalButton("Cancel"))
    ))
    
  })
  
  observeEvent(input$set_delete_room_confirm,{
    req(input$remove_room_list)
    fishdb<-readRDS("fishdatabase.rds")
    nursery_rooms<-fishdb[["nursery_info"]][,1]
    adult_rooms<-fishdb[["adult_info"]][,1]
    if(is.element(input$remove_room_list,nursery_rooms)){
      ...
      if(permission=="No"){
        output$set_remove_room_error<- renderText(
          "ERROR !!! You cannot remove this room as it is important for database functioning."
        ...
        fishdb[["nursery_info"]]<-fishdb[["nursery_info"]][-info_loc,]
        fishdb[[db_loc]]<-NULL
        ...
        log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                          NA,NA, NA,
                                          NA,
                                          paste0(toupper(user_info()$name)," REMOVED THE FISH ROOM ", toupper(input$remove_room_list), " FROM THIS
                                             DATABASE"))
        fishdb$Database_logs<-log_data
        saveRDS(fishdb, "fishdatabase.rds")
        ...
        showModal(modalDialog(
          tagList(
            ...
            paste0("The fish room is successfully removed from this database !!")
          ),
          title = p(style= "color:green",strong("SUCCESS !! ")),
          footer = tagList(actionButton("set_remove_room_success1","Close"))
        ))
        updateSelectizeInput(session, inputId = "remove_room_list", choices = NULL, server = TRUE)
        
      }
    }else if(is.element(input$remove_room_list,adult_rooms)){
      info_loc<-which(input$remove_room_list==adult_rooms)
      db_loc<-fishdb[["adult_info"]][info_loc,2]
      permission<-fishdb[["adult_info"]][info_loc,3]
      if(permission=="No"){
        output$set_remove_room_error<- renderText(
          "ERROR !!! You cannot remove this room as it is important for database functioning."
        ...
        fishdb[["adult_info"]]<-fishdb[["adult_info"]][-info_loc,]
        fishdb[[db_loc]]<-NULL
        ...
        showModal(modalDialog(
          ...
          title = p(style= "color:green",strong("SUCCESS !! ")),
          footer = tagList(actionButton("set_remove_room_success2","Close"))
        ))
        updateSelectizeInput(session, inputId = "remove_room_list", choices = NULL, server = TRUE)
  ...
  observeEvent(input$set_remove_room_success1,{
    shinyjs::hide("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::hide("set_rename_room")
    removeModal()
  })
  observeEvent(input$set_remove_room_success2,{
    shinyjs::hide("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::hide("set_rename_room")
    removeModal()
  ...
  #######################################Server side code for renaming the fish Room ###########################################################
  observeEvent(input$rename_fish_room,{
    fishdb<-readRDS("fishdatabase.rds")
    rooms<-c(fishdb[["nursery_info"]][,1],fishdb[["adult_info"]][,1])
    updateSelectizeInput(session, inputId = "rename_room_list", choices = rooms, server = TRUE)
    shinyjs::hide("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::show("set_rename_room")
  })
  
  observeEvent(input$rename_room_submit,{
    req(input$rename_room_list)
    fishdb<-readRDS("fishdatabase.rds")
    user_inputs<-c(input$rename_room_list, input$rename_room_name)
    qc<-check_errors(user_inputs)
    rooms<-c(fishdb[["nursery_info"]][,1],fishdb[["adult_info"]][,1])
    check_name_dup<- dup_check(input$rename_room_name,rooms)
    if(qc==T){
      output$set_rename_room_error<- renderText(
        "ERROR !!! Please fill all the *mandatory fields."
      ...
      showModal(modalDialog(
        tagList(
          paste0("Please confirm renaming ",input$rename_room_list, " to ",input$rename_room_name )
        ),
        title = strong("CONFIRM"),
        footer = tagList(actionButton("set_rename_room_confirm","Confirm"), modalButton("Cancel"))
      ))
    }
    
  })
  
  observeEvent(input$set_rename_room_confirm,{
    req(input$rename_room_list)
    req(input$rename_room_name)
    fishdb<-readRDS("fishdatabase.rds")
    nursery_rooms<-fishdb[["nursery_info"]][,1]
    adult_rooms<-fishdb[["adult_info"]][,1]
    if(is.element(input$rename_room_list,nursery_rooms)){
      info_loc<-which(input$rename_room_list==nursery_rooms)
      fishdb[["nursery_info"]][info_loc,1]<-input$rename_room_name

    }else if(is.element(input$rename_room_list,adult_rooms)){
      info_loc<-which(input$rename_room_list==adult_rooms)
      fishdb[["adult_info"]][info_loc,1]<-input$rename_room_name
    }
    log_data<-fishdb[["Database_logs"]]
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," RENAMED ", toupper(input$rename_room_list), " TO ", toupper(input$rename_room_name)))
    fishdb$Database_logs<-log_data
    saveRDS(fishdb, "fishdatabase.rds")
    ...
    showModal(modalDialog(
      tagList(
        ...
        paste0("The fish room is successfully renamed !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("set_rename_room_success","Close"))
    ))
    updateSelectizeInput(session, inputId = "rename_room_list", choices = NULL, server = TRUE)
    
  })
  
  observeEvent(input$set_rename_room_success,{
    shinyjs::hide("set_create_room")
    shinyjs::hide("set_remove_room")
    shinyjs::hide("set_rename_room")
    removeModal()
  ...
  ###################################################Served Side Code for Editing User Accounts#################################################
  fishdb<-readRDS("fishdatabase.rds")
  user_base<- fishdb$user_base

  observeEvent(input$add_new_user,{
    shinyjs::show("set_new_user")
    shinyjs::hide("set_remove_user")
    shinyjs::hide("set_change_pwd")
  })
  observeEvent(input$remove_user,{
    ...
    updateSelectizeInput(session, inputId = "delete_user", choices = setdiff( user_base$user,"Administrator"), server = TRUE)
    updateSelectizeInput(session, inputId = "change_pwd_user", choices = user_base$user, server = TRUE)
    shinyjs::hide("set_new_user")
    shinyjs::show("set_remove_user")
    shinyjs::hide("set_change_pwd")
  })
  ...
  observeEvent(input$new_user_submit,{
    ...
    if(qc==T){
      output$set_new_user_error<- renderText(
        "ERROR !!! Please fill all the *mandatory fields."
    ...
    }else if(input$new_pwd!=input$new_pwd_confirm){
      output$set_new_user_error<- renderText(
      ...
      user_base<- fishdb$user_base
      user_base<-user_base %>%
        add_row(user = input$new_user_name, password = input$new_pwd_confirm, permissions = input$new_user_permission,
                 name = input$new_user_name)
      fishdb$user_base<-user_base
      log_data<-fishdb[["Database_logs"]]
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                        NA,NA, NA,
                                        NA,
                                        paste0(toupper(user_info()$name)," ADDED A NEW USER:  ", toupper(input$new_user_name), " TO THIS DATABASE"))
      fishdb$Database_logs<-log_data
      saveRDS(fishdb, "fishdatabase.rds")
      showModal(modalDialog(
        tagList(

          paste0("NEW USER ADDED TO THE DATABASE !!")
        ),
        title = p(style= "color:green",strong("SUCCESS !! ")),
        footer = tagList(actionButton("set_new_user_success","Close"))
      ))
      
      updateSelectizeInput(session, inputId = "delete_user", choices = NULL, server = TRUE)
      updateSelectizeInput(session, inputId = "change_pwd_user", choices = NULL, server = TRUE)

    }
  })
  
  observeEvent(input$set_new_user_success,{
    shinyjs::hide("set_new_user")
    shinyjs::hide("set_remove_user")
    shinyjs::hide("set_change_pwd")
    removeModal()
  ...
  ########################################### removing a user ###################################################################
  observeEvent(input$delete_user_submit,{
    req(input$delete_user)
    showModal(modalDialog(
      tagList(
        paste0("Please confirm removing the user ",input$delete_user, " from this fish database."),
      ),
      title = tags$strong(tags$span("CAUTION !!! CONFIRM REMOVING USER", style = "color: red;")),
      footer = tagList(actionButton("set_remove_user_confirm","Confirm"), modalButton("Cancel"))
    ))
  })
  
  observeEvent(input$set_remove_user_confirm,{
    req(input$delete_user)
    fishdb<-readRDS("fishdatabase.rds")
    user_base<- fishdb$user_base
    user_base <- user_base %>%
      filter(user != input$delete_user)
    fishdb$user_base<-user_base
    log_data<-fishdb[["Database_logs"]]
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," REMOVED THE USER:  ", toupper(input$delete_user), " FROM THIS DATABASE"))
    fishdb$Database_logs<-log_data
    saveRDS(fishdb, "fishdatabase.rds")
    showModal(modalDialog(
      tagList(
        
        paste0("SUCCESSFULLY REMOVED USER FROM THE DATABASE !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("set_remove_user_success","Close"))
    ))
    updateSelectizeInput(session, inputId = "delete_user", choices = NULL, server = TRUE)
    updateSelectizeInput(session, inputId = "change_pwd_user", choices = NULL, server = TRUE)
  })
  
  observeEvent(input$set_remove_user_success,{
    shinyjs::hide("set_new_user")
    shinyjs::hide("set_remove_user")
    shinyjs::hide("set_change_pwd")
    removeModal()
  ...
  ########################################## changing password ########################################################################
  observeEvent(input$change_pwd_submit,{
    req(input$change_pwd_user)
    user_inputs<-c(input$change_pwd_user,input$change_pwd, input$change_pwd_confirm)
    qc<-check_errors(user_inputs)
    if(qc==T){
      output$set_change_pwd_error<- renderText(
        "ERROR !!! Please fill all the *mandatory fields."
        
      )
    }else if(input$change_pwd!=input$change_pwd_confirm){
      output$set_change_pwd_error<- renderText(
        "ERROR !!! Your passwords don't match."
      ...
      fishdb<-readRDS("fishdatabase.rds")
      user_base<- fishdb$user_base
      user_base<-user_base %>% mutate(password = if_else(user == input$change_pwd_user, input$change_pwd_confirm, password))
      fishdb$user_base<-user_base
      log_data<-fishdb[["Database_logs"]]
      log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                        NA,NA, NA,
                                        NA,
                                        paste0(toupper(user_info()$name)," CHANGED THE PASSWORD FOR THE USER:  ", toupper(input$change_pwd_user)))
      fishdb$Database_logs<-log_data
      saveRDS(fishdb, "fishdatabase.rds")
      showModal(modalDialog(
        tagList(
          
          paste0("PASSWORD CHANGED SUCCESSFULLY !!")
        ),
        title = p(style= "color:green",strong("SUCCESS !! ")),
        footer = tagList(actionButton("set_change_pwd_success","Close"))
      ))
    }
  })
  
  
  observeEvent(input$set_change_pwd_success,{
    shinyjs::hide("set_new_user")
    shinyjs::hide("set_remove_user")
    shinyjs::hide("set_change_pwd")
    removeModal()
  ...
  observeEvent(input$clean_slate_submit,{
    ...
    if(length(input$clean_slate_list)>0){
      ...
      showModal(modalDialog(
        tagList(
          strong(paste0("Please confirm deleting all data from the following file/s : ",paste0(input$clean_slate_list, collapse = " , ")))
          
        ),
        title = tags$strong(tags$span("CAUTION !!! CONFIRM", style = "color: red;")),
        footer = tagList(actionButton("clean_slate_confirm","Confirm"), modalButton("Cancel"))
      ))
      
    }else{
      output$clean_state_error<-renderText(
        "ERROR !!! Please select atleast one input."
      )
    }
  })
  
  
  observeEvent(input$clean_slate_confirm,{
    fishdb<-readRDS("fishdatabase.rds")
    for (i in input$clean_slate_list) {
      if(i=="Nursery Fish Rooms"){
        info<-fishdb$nursery_info
        dbnames<-info$db_list_name
        for (j in dbnames){
          df<-fishdb[[j]]
          df<-df[0,]
          fishdb[[j]]<-df
        }
      } else if(i=="Adult Fish Rooms"){
        info<-fishdb$adult_info
        dbnames<-info$db_list_name
        for (j in dbnames){
          col_names<-c("Tank.Name" ,       "Stock.number." ,   "Date.of.Birth" ,   "Fish.name",        "Genotype."  ,      "Responsible" ,     "Number.of.fish",   "Experiment.label",
                       "Food.label" ,      "Notes"  )
        ...
        logdata<-fishdb$Database_logs
        logdata <- logdata[0, ]
        fishdb$Database_logs<-logdata
      }else if(i=="Archives"){
        arch<-fishdb$Archive
        arch <- arch[0, ]
        fishdb$Archive<-arch
    ...
    log_data[nrow(log_data)+1,]<-list(format(Sys.time(), "%d-%b-%Y %H.%M"),NA, NA,
                                      NA,NA, NA,
                                      NA,
                                      paste0(toupper(user_info()$name)," PERFORMED CLEAN SLATE PROTOCOL ON FOLLOWING FILES:  ", paste0(input$clean_slate_list, collapse = " , ")))
    fishdb$Database_logs<-log_data
    saveRDS(fishdb, "fishdatabase.rds")
    ...
    showModal(modalDialog(
      tagList(
        ...
        paste0("CLEAN SLATE PROTOCOL SUCCESSFULLY COMPLETED !!")
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("clean_slate_success","Close"))
    ))
    
  })
  
  observeEvent(input$clean_slate_success,{
    removeModal()
  })
  
  rds_file_path <- "fishdatabase.rds"
  
  output$create_backup <- downloadHandler(
    filename = function() {
      paste0("fishdatabase_backup_",Sys.Date(),".rds")
      
    },
    content = function(file) {
      file.copy(rds_file_path, file)  # Copy the file from server to user
    },
    contentType = "application/octet-stream"  # Ensure correct MIME type for zip files
  )

  #####################################################################################################################################################
  ################################################Server side for restoring database###############################################################

  observeEvent(input$restore_rds,{
    req(input$rdsFile)
    rdsfile<-readRDS(input$rdsFile$datapath)
    qc<-check_rds_format(rdsfile)
    if(qc==TRUE){
      output$restore_rds_error<- renderText(
        ""
      )
      showModal(modalDialog(
        tagList(
          strong(
            paste0("Please confirm restoring the database using this file : ",input$rdsFile$name)
          )
        ),
        title = p(style="color:green",strong("CONFIRM THE RDS FILE TO RESTORE")),
        footer = tagList(actionButton("restore_rds_confirm","Confirm"), modalButton("Cancel"))
      ))
      
    }else{
      output$restore_rds_error<- renderText(
        "ERROR !!! This rds file is corrupted and not in the correct format !!!"
      )
    }
  })
  
  observeEvent(input$restore_rds_confirm,{
    req(input$rdsFile)
    rdsfile<-readRDS(input$rdsFile$datapath)
    saveRDS(rdsfile,"fishdatabase.rds")
    removeModal()
    
    showModal(modalDialog(
      tagList(
        strong(
          paste0("You successfully restored this fish database !!")
        )
      ),
      title = p(style= "color:green",strong("SUCCESS !! ")),
      footer = tagList(actionButton("restore_rds_modal_success","Close"))
    ))
    
  })
  
  observeEvent(input$restore_rds_modal_success,{
    removeModal()
  })
 
  
   
}###############End bracket for the server################################################################
